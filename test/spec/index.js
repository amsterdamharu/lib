...
const createPromise = () => {
  ...
  promise.catch = reject => promise.then(undefined, reject);
...
const rejectLater = (rejectValue, time = 500) =>
  lib.queReject(
    new Promise((resolve, reject) => setTimeout(x => reject(rejectValue), time))
  );
...
const shouldNotReject = resolve(x => expect("Should not reject").toBe(false));
...
describe("anyPromise", function() {
  [
    ...
    },
    {
      ...
      resolve: resolve(x =>
        expect("Resolve should not be called when empty array is passed").toBe(
          false
        )
      ),
      reject: reject(x => expect(x).not.toBeNull())
    ...
    it(config.desc, done => {
      lib
        .anyPromise(
          config.param &&
            config.param.map(x => (typeof x === "function" ? x() : x))
        )
...
describe("throtle", function() {
  ...
  it("resolve at the right moment", done => {
    const start = [],
      finish = [];
    [1, 2, 3, 4, 5].map(x => max2(doStart(50)(start)(finish))(x));
    later(false, 25)
      .then(
        resolve(x => {
...
describe("compose", function() {
  it("Should not resolve as promise if all functions are sync", () => {
    expect(lib.compose([x => x + 1, x => x + 2, x => x + 3])(0)).toBe(6);
  });
  it("Should resolve as promise if some are promises", done => {
    lib
      .compose([x => later(x + 1, 50), x => x + 2, x => x + 3])(0)
      .then(resolve(x => expect(x).toBe(6))(done), shouldNotReject);
  });
...
describe("result", function() {
  [
    ...
    ["Should create a result with value 22", lib.result.success(22).value, 22],
    [
  ...
  [
    {
      ...
      resolve: resolve(x =>
        expect("Resolve should not be called when timed out").toBe(false)
      ),
    ...
    },
    {
      ...
      resolve: resolve(x =>
        expect("Resolve should not be called when function rejects").toBe(false)
      ),
    ...
    },
    {
      ...
      resolve: resolve(x =>
        expect("Resolve should not be called when function throws").toBe(false)
      ),
...
describe("throttlePeriod", function() {
  ...
  it("resolve period throttle at the right moment", done => {
    const start = [],
      finish = [];
    [1, 2, 3, 4, 5].map(x => twoPer50(doStart(50)(start)(finish))(x));
    later(false, 25)
      .then(
        resolve(x => {
          expect(start.join()).toBe("1,2");
...
describe("memoize", function() {
  ...
  it("Should not call the memoized function", () => {
    expect(memCalled).toBe(1);
  });
...
const getData = url => {
  ...
  return Promise.resolve(url)
    ...
    .then(objectWithDates) //does no use threaded valueme
